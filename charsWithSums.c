...
void smoothBlurFilter(unsigned char *pixel1, unsigned char *c, int *sums) {
...
void imageToCharsAndSum(unsigned char *data, unsigned char* c, int* sums) {
    int i, j, sum;
...
void myfunction(Image *image, char* srcImgpName, char* blurRsltImgName, char* sharpRsltImgName, char* filteredBlurRsltImgName, char* filteredSharpRsltImgName, char flag) {
    ...
    if (flag =='1') {
        ...
        int *sums = (int *) malloc(NM4);
        char *backupOrg = malloc(NM3);
        imageToCharsAndSum(image->data, backupOrg, sums);
        char *c = image->data + M3PLUS3;
        smoothBlurFilter(backupOrg + 3, c, sums);
        imageToChars(image->data, backupOrg);
